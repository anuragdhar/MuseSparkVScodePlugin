...
function activate(context) {
    ...
    const costTracker = costTracker_1.CostTracker.init(context);
    const billingTracker = billingTracker_1.BillingTracker.init(context);
    context.subscriptions.push(vscode.lm.registerLanguageModelChatProvider('muse-spark', new languageModelProvider_1.MuseLanguageModelProvider(context.secrets)), vscode.commands.registerCommand('museSpark.manageLanguageModelProvider', () => (0, languageModelProvider_1.configureMuseLanguageModel)(context.secrets)));
    const client = new sparkClient_1.SparkClient();
    chatProvider = new chatPanel_1.ChatViewProvider(context.extensionUri, context);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(chatPanel_1.ChatViewProvider.viewType, chatProvider, { webviewOptions: { retainContextWhenHidden: true } }));
    ...
    context.subscriptions.push(costTracker.onDidUpdate(totals => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('museSpark.stopAgent', () => chatProvider.stopAgent()), vscode.commands.registerCommand('museSpark.newSession', () => chatProvider.newSession()), vscode.commands.registerCommand('museSpark.forkSession', () => chatProvider.forkSession()), vscode.commands.registerCommand('museSpark.listSessions', async () => chatProvider.listSessions()), vscode.commands.registerCommand('museSpark.sessionStats', async () => {
        ...
        if (all.length > 20)
            lines.push(`_…and ${all.length - 20} more_`);
        // Also render in chat
        chatProvider.showStatsInChat?.();
        ...
        if (!pick)
            return;
        const ok = await vscode.window.showWarningMessage(`Delete "${pick.label}"?`, { modal: true }, 'Delete');
        if (ok === 'Delete')
            await chatProvider.deleteSession(pick.id);
        ...
        if (!pick)
            return;
        const title = await vscode.window.showInputBox({ prompt: 'New title', value: pick.label });
        if (title)
            await chatProvider.renameSession(pick.id, title);
        ...
        if (!pick)
            return;
        await chatProvider.exportSession(pick.id);
        ...
        try {
            ...
            const repo = git?.getRepository(vscode.workspace.workspaceFolders?.[0]?.uri);
            if (repo) {
                await repo.clean(repo.state.workingTreeChanges.map((c) => c.uri));
                vscode.window.showInformationMessage('Reverted working tree changes.');
                return;
            }
        }
        catch { }
        ...
        if (!pick)
            return;
        await vscode.workspace.getConfiguration('museSpark').update('approvalMode', pick.value, vscode.ConfigurationTarget.Global);
        // migrate legacy
        await vscode.workspace.getConfiguration('museSpark').update('cliFullAccess', pick.value === 'fullAccess', vscode.ConfigurationTarget.Global);
        ...
        const cfg = vscode.workspace.getConfiguration('museSpark');
        const current = cfg.get('useContributorPricing') ?? true;
        ...
        vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Muse Spark: refreshing live billing…' }, async () => {
            ...
            if (info?.error)
                vscode.window.showWarningMessage(`Billing refresh: ${info.error}`);
            else
                vscode.window.showInformationMessage(`Live unpaid: $${(info?.unpaid ?? 0).toFixed(4)} ${info?.currency ?? 'USD'}`);
        });
    ...
    const inlineProvider = {
        async provideInlineCompletionItems(document, position, context, token) {
            const enabled = vscode.workspace.getConfiguration('museSpark').get('enableInlineCompletion');
            if (!enabled)
                return;
            if (context.triggerKind === vscode.InlineCompletionTriggerKind.Automatic && Math.random() > 0.5)
                return;
            const prefix = document.getText(new vscode.Range(new vscode.Position(0, 0), position));
            if (prefix.length < 20)
                return;
            const line = document.lineAt(position).text.slice(0, position.character);
            if (!line.trim() && prefix.trim().length < 50)
                return;
            try {
...
async function runTransform(title, fn) {
    const sel = getSelection();
    if (!sel)
        return;
