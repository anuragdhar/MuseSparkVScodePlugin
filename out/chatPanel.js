...
class ChatViewProvider {
    ...
    async listSessions() {
        if (!this.sessionStore) {
            const cfg = vscode.workspace.getConfiguration('museSpark');
            vscode.window.showInformationMessage(`Spark session: model=${cfg.get('model')} • backend=${cfg.get('backend')} • approval=${(0, sessionStore_1.resolveApprovalMode)()} • id=${this.activeSessionId.slice(0, 8)} • msgs=${this.messages.length}`);
            return;
        }
    ...
    resolveWebviewView(view) {
        ...
        setTimeout(() => {
            for (const m of this.messages.slice(-50))
                this.post({ type: 'addMessage', role: m.role, content: m.content });
